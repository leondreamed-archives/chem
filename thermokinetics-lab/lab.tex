...
const data = rawData.map(
	([volume, initialTemp, zincAdditionTime, coolingLineB, coolingLineM, rValue]) => ({
		volume,
		initialTemp,
		zincAdditionTime,
		coolingLineB,
...
function getRowEquation(rowIndex) {
	...
	const equation = `${tsf(row.coolingLineM)} ${sign} ${tsf(Math.abs(row.coolingLineM))}$x$`;
	return equation;
}
?>
% \end{noindent}
...
const avgHReaction = _.mean(hReactions);
...
const fullMean = _.mean(hReactions);
const filteredMean = _.mean(filteredHReactions);
const numerator = _.sum(filteredHReactions.map(y => (y - filteredMean) ** 2));
const denominator = _.sum(hReactions.map(y => (y - fullMean) ** 2));
...
%
<?
const avgHReaction2 = _.mean(filteredHReactions);
